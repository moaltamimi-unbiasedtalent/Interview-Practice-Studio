...
def test_harness_runs_and_reports_expected_structure() -> None:
    assert evq.main() == 0
    report = json.loads((ROOT / "evaluations/quality_v2/results.json").read_text())
...
def test_ood_overlap_is_below_in_domain() -> None:
    evq.main()
    ood = json.loads((ROOT / "evaluations/quality_v2/results.json").read_text())["out_of_domain"]

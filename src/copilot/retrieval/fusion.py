...
def reciprocal_rank_fusion(
    ...
    *,
    ...
    Duplicate chunks (same ``chunk_id``) are merged; the fused score is the RRF
    sum across lists. Ties break deterministically by chunk id so results are
    reproducible. The returned :class:`RetrievalResult`s carry the fused score
    and ``retriever="fusion"``.
    """
    ...
    for chunk_id, score in ordered:
        ...
        fused.append(
            RetrievalResult(chunk=source.chunk, score=score, retriever="fusion")
        )
